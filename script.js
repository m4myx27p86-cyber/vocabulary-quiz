...
// 初期コードごとに、利用できる教材を指定します。
// 管理者はここを編集して、学習者ごとに初期コードを配布してください。
const INITIAL_ACCESS_CODES = {
  "2468": { name: "英語基礎", materials: ["vocab", "sentence"] },
  "1357": { name: "TOEIC", materials: ["monitor", "speakingReview", "calendar"] },
  "8642": { name: "古典", materials: ["classicalWords", "classicalGrammar", "classicalKnowledge"] },
  "5555": { name: "全教材", materials: ["vocab", "sentence", "monitor", "speakingReview", "classicalWords", "classicalGrammar", "classicalKnowledge", "calendar"] }
};

// true にすると、許可された教材でも教材パスワード入力を要求します。
// 学習者ごとのアクセス制限だけで運用する場合は false のままで大丈夫です。
const REQUIRE_MATERIAL_PASSWORD = false;
const GAS_WEB_APP_URL = ""; // Google Apps Script のWebアプリURLを入れるとスプレッドシート送信が有効になります。

const STORAGE_KEYS = {
  sessionStudent: "learningTool_sessionStudent",
  answerHistory: "learningTool_answerHistory",
  learners: "learningTool_registeredLearners",
...
let currentAllowedMaterials = [];
let isAdmin = false;
let pendingSetupCode = "";
...
document.addEventListener("DOMContentLoaded", () => {
  safeAddEvent("loginButton", "click", checkMainLogin);
  safeAddEvent("passwordInput", "keydown", event => { if (event.key === "Enter") checkMainLogin(); });
  safeAddEvent("showPassword", "change", function () { togglePasswordField("passwordInput", this.checked); });

  safeAddEvent("registerButton", "click", registerNewLearner);
  safeAddEvent("registerBackButton", "click", () => showOnly("loginScreen"));
  safeAddEvent("showRegisterPassword", "change", function () {
    togglePasswordField("registerPasswordInput", this.checked);
    togglePasswordField("registerPasswordConfirm", this.checked);
  });
...
function getAllAccessItems() {
  return [...Object.keys(TEST_CONFIG), "calendar"];
}

function getRegisteredLearners() {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEYS.learners)) || [];
  } catch {
    return [];
  }
}

function saveRegisteredLearners(learners) {
...
function checkMainLogin() {
  ...
  if (!/^\d{4}$/.test(input)) {
    document.getElementById("loginMessage").textContent = "ログインできません。管理者に確認してください。";
    return;
  }
  ...
  const learner = learners.find(item => item.password === input);
  if (learner) {
    loginAsLearner({
      studentId: learner.studentId,
      name: learner.name || learner.studentId,
  ...
  if (INITIAL_ACCESS_CODES[input]) {
    pendingSetupCode = input;
    document.getElementById("registerNameInput").value = INITIAL_ACCESS_CODES[input].name || "";
...
function registerNewLearner() {
  ...
  const codeInfo = INITIAL_ACCESS_CODES[pendingSetupCode];

  if (!codeInfo) {
    message.textContent = "初期コードを確認できません。最初からやり直してください。";
    return;
  }

  if (!/^\d{4}$/.test(password)) {
    message.textContent = "4桁の数字で設定してください。";
    return;
  }
  ...
  if (password === ADMIN_PASSWORD || INITIAL_ACCESS_CODES[password]) {
    message.textContent = "この番号は使用できません。別の4桁を設定してください。";
    return;
  }

  const learners = getRegisteredLearners();
  if (learners.some(item => item.password === password)) {
    message.textContent = "この番号はすでに登録されています。別の4桁を設定してください。";
    return;
  }

  const studentId = `S${String(learners.length + 1).padStart(4, "0")}`;
  const learner = {
    studentId,
    name: displayName || codeInfo.name || studentId,
    password,
    setupCode: pendingSetupCode,
    materials: codeInfo.materials || [],
    createdAt: new Date().toLocaleString("ja-JP")
  };

  learners.push(learner);
  saveRegisteredLearners(learners);
  pendingSetupCode = "";
...
function loginAsLearner({ studentId, name, materials, isAdminUser }) {
  currentStudentId = studentId;
  currentLearnerName = name || studentId;
  currentAllowedMaterials = isAdminUser ? getAllAccessItems() : [...materials];
  isAdmin = Boolean(isAdminUser);
  localStorage.setItem(STORAGE_KEYS.sessionStudent, studentId);

  document.getElementById("passwordInput").value = "";
  document.getElementById("studentLoginInput").value = "";
  showOnly("menuScreen");
  refreshDashboard();
  updateMaterialAccessUI();
}
...
function userCanAccessMaterial(type) {
  return isAdmin || currentAllowedMaterials.includes(type);
}

function updateMaterialAccessUI() {
  document.querySelectorAll(".material-card[data-material]").forEach(button => {
    const type = button.dataset.material;
    const allowed = userCanAccessMaterial(type);
    button.disabled = !allowed;
    button.classList.toggle("locked-card", !allowed);
    const small = button.querySelector("small");
    if (small) small.textContent = allowed ? "利用できます" : "現在は利用できません";
  });

  const calendarButton = document.getElementById("calendarButton");
  if (calendarButton) {
    const allowed = userCanAccessMaterial("calendar");
    calendarButton.disabled = !allowed;
    calendarButton.classList.toggle("locked-card", !allowed);
    const small = calendarButton.querySelector("small");
    if (small) small.textContent = allowed ? "利用できます" : "現在は利用できません";
  }
}
...
function openMaterialPassword(type) {
  ...
  if (!userCanAccessMaterial(type)) {
    alert("この教材は現在利用できません。管理者に確認してください。");
    return;
  }

  if (!REQUIRE_MATERIAL_PASSWORD || isAdmin) {
    openSettings(type);
    return;
  }
...
function checkMaterialPassword() {
  ...
  if (!userCanAccessMaterial(pendingMaterialType)) {
    document.getElementById("materialPasswordMessage").textContent = "この教材は現在利用できません。";
    return;
  }
  ...
  if (input === config.password || isAdmin) {
    ...
    document.getElementById("materialPasswordMessage").textContent = "利用できません。管理者に確認してください。";
  }
}
...
async function startReviewQuiz() {
  ...
  reviewMode = true;
  const sourceQuestions = await ensureQuestionsLoaded("vocab");
...
function showChoiceQuestion() {
  const q = questions[currentIndex];
  document.getElementById("testTitle").textContent = reviewMode ? "Stock 3000 間違い復習" : TEST_CONFIG[testType].title;
...
function openHistoryScreen() {
  if (!isAdmin) {
    alert("解答履歴は管理者のみ閲覧できます。管理者番号 9999 でログインしてください。");
    return;
  }
...
function getWrongKey() {
  return `wrongWords_${currentStudentId || "unknown"}`;
}
...
async function openCalendar() {
  if (!userCanAccessMaterial("calendar")) {
    alert("学習カレンダーは現在利用できません。管理者に確認してください。");
    return;
  }
...
function refreshDashboard() {
  ...
  document.getElementById("studentStatus").textContent = `${role}：${currentLearnerName || currentStudentId}（${currentStudentId}）`;
