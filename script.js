/* =========================================================
   学習ツール script.js
   - 初回登録: 初期コード入力後、個別4桁パスワードを登録
   - 通常ログイン: 登録済みの個別4桁パスワード / 管理者番号
   - 教材ごとの4桁パスワード
   - localStorage履歴 + Google Spreadsheet送信拡張対応
========================================================= */

const INITIAL_SETUP_CODE = "2468"; // 初回登録用コード。必要に応じて変更してください。
const ADMIN_PASSWORD = "9999";
const GAS_WEB_APP_URL = ""; // Google Apps Script の WebアプリURLを入れるとスプレッドシート送信が有効になります。

const STORAGE_KEYS = {
  sessionStudent: "learningTool_sessionStudent",
  studentAccounts: "learningTool_studentAccounts",
...
document.addEventListener("DOMContentLoaded", () => {
  safeAddEvent("loginButton", "click", checkLoginCode);
  safeAddEvent("passwordInput", "keydown", event => { if (event.key === "Enter") checkLoginCode(); });
  safeAddEvent("showPassword", "change", function () { togglePasswordField("passwordInput", this.checked); });

  safeAddEvent("registerButton", "click", registerStudentPassword);
  safeAddEvent("registerPasswordConfirm", "keydown", event => { if (event.key === "Enter") registerStudentPassword(); });
...
function checkLoginCode() {
  ...
  if (input === ADMIN_PASSWORD) {
    currentStudentId = "管理者";
    isAdmin = true;
    localStorage.setItem(STORAGE_KEYS.sessionStudent, "admin");
    message.textContent = "";
    showOnly("menuScreen");
    refreshDashboard();
    return;
  ...
  const account = getStudentAccounts().find(item => item.password === input);
  if (account) {
    currentStudentId = account.id;
    isAdmin = false;
    localStorage.setItem(STORAGE_KEYS.sessionStudent, account.id);
    message.textContent = "";
    showOnly("menuScreen");
    refreshDashboard();
    return;
...
function clearRegisterForm() {
  ["registerNameInput", "registerPasswordInput", "registerPasswordConfirm"].forEach(id => {
    const element = document.getElementById(id);
...
function registerStudentPassword() {
  ...
  if (!/^\d{4}$/.test(password)) {
    message.textContent = "個別パスワードは4桁の数字で設定してください。";
    return;
  }

  if (password === INITIAL_SETUP_CODE || password === ADMIN_PASSWORD) {
  ...
  if (accounts.some(item => item.password === password)) {
    message.textContent = "この個別パスワードはすでに登録されています。別の4桁を設定してください。";
    return;
  }

  const account = {
    id: createStudentId(accounts),
    name: name || "未設定",
    password,
...
function createStudentId(accounts) {
...
function getStudentAccounts() {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEYS.studentAccounts)) || [];
  } catch {
    return [];
  }
}
...
function checkStudentLogin() {
  ...
  if (!/^\d{4}$/.test(input)) {
    document.getElementById("studentLoginMessage").textContent = "4桁の数字を入力してください。";
    return;
  }

  const account = getStudentAccounts().find(item => item.password === input);
  if (!account && input !== ADMIN_PASSWORD) {
    document.getElementById("studentLoginMessage").textContent = "登録済みの個別パスワードと一致しません。";
    return;
  }

  currentStudentId = input === ADMIN_PASSWORD ? "管理者" : account.id;
...
function logout() {
  ...
  ["passwordInput", "studentLoginInput", "registerNameInput", "registerPasswordInput", "registerPasswordConfirm"].forEach(id => {
    const element = document.getElementById(id);
    if (element) element.value = "";
  });
...
function togglePasswordField(id, checked) {
  ...
  if (!field) return;
  field.classList.toggle("visible-code", checked);
}
...
function openHistoryScreen() {
  if (!isAdmin) {
    alert("解答履歴は管理者のみ閲覧できます。管理者用の番号でログインしてください。");
    return;
  }
...
function refreshDashboard() {
  ...
  const account = getStudentAccounts().find(item => item.id === currentStudentId);
  const displayName = account && account.name !== "未設定" ? ` / ${account.name}` : "";
  document.getElementById("studentStatus").textContent = `${role}：${currentStudentId}${displayName}`;
